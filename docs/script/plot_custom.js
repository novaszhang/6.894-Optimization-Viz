...
function get_f_values(nx, ny, func) {
    ...
    for (i = 0; i < nx; i++) {
        for (j = 0; j < ny; j++) {
            ...
            grid[i + j * nx] = parseFloat(math.evaluate(func, {x:x, y:y}));
        }
    }
    return grid;
}
...
function grad_f(x,y, grad_x, grad_y) {
  let scope = {x:x, y:y}
...
//   if (draw_bool.SGD) {
//     var sgd_data = get_sgd_path(x0, y0, sgd_lr-2);
...
function minimize(
  ...
  if (draw_bool.Momentum) {
      var momentum_data = get_momentum_path(x0, y0, mom_lr, 700, 0.8);
      draw_path(momentum_data, "momentum");
  }
  if (draw_bool.RMSProp) {
      var rmsprop_data = get_rmsprop_path(x0, y0, rms_lr, 700, 0.99, 1e-6);
      draw_path(rmsprop_data, "rmsprop");
  }
  if (draw_bool.Adam) {
      var adam_data = get_adam_path(x0, y0, adam_lr, 700, 0.7, 0.999, 1e-6);
      draw_path(adam_data, "adam");
  }
}
